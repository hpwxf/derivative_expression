...
ScalarData S2SFunction::operator()(const ScalarData x) const { return m_f->eval(x); }
S2SFunction S2SFunction::diff() const { return m_f->diff(); }
std::ostream &S2SFunction::print(std::ostream &o) const { return m_f->print(o); }
int S2SFunction::memory_size() const { return sizeof(*this) + m_f->memory_size(); }

// Quelques fonctions pour l'interface utilisateur
S2SFunction sin(const S2SFunction &f) { return S2SFunction(new Sinus(f)); }
S2SFunction cos(const S2SFunction &f) { return S2SFunction(new Cosinus(f)); }
...
S2SFunction operator+(const ScalarData &a, const S2SFunction &f) { return f + a; }
S2SFunction operator*(const S2SFunction &f, const ScalarData &a) { return a * f; }
S2SFunction operator/(const S2SFunction &f, const ScalarData &a) { return f * (1. / a); }
S2SFunction operator-(const ScalarData &a, const S2SFunction &f) { return f - a; }
S2SFunction operator-(const S2SFunction &f) { return -1 * f; }
S2SFunction operator-(const S2SFunction &f, const S2SFunction &g) { return f + (-g); }
...
S2SFunction Identity::diff() const { return new Constant(1); }
S2SFunction Constant::diff() const { return new Constant(0); }
S2SFunction Sinus::diff() const { return cos(m_f) * m_f->diff(); }
S2SFunction Cosinus::diff() const { return -sin(m_f) * m_f->diff(); }
S2SFunction Add::diff() const { return m_f->diff() + m_g->diff(); }
...
S2SFunction d(const S2SFunction &f) { return f.diff(); }
